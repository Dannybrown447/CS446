 ...
 *			assignment 5 of the CS 446/646 course simulator.
 *
 * @author Daniel Brown
 *
 * @version 2.00 (10 March 17)
 * @version 3.00 (14 April 17)
 */
...
bool Simulator:: runSim(int argc, char *argv[])
{
	...
	int counter = 1;
	int tempCheck = 0;
	std::vector<cycleInfo> FIFOVec;
	std::queue<std::vector<cycleInfo> > processQueue;
	int j = 0;
	...
	*/
	for(int i = 0; i < cycle.size(); i++)
	{
   	...
   	////// Create Process Queue based on Config File ////////////
   	   else if(cycle[i].id == "FIFO")
   	   {
   	   	 processQueue = createFIFOQueue(cycle);
   	   }
   	   else if(cycle[i].id == "SJF")
   	   {
   	   	
   	   	 processQueue = createSJFqueue(cycle);
   	   }
   	   else if(cycle[i].id == "PS")
   	   {

   	   	 processQueue = createPSqueue(cycle);
   	   }
   	   else if(cycle[i].id == "RR")
   	   {
   	   	std::cerr<<"Round Robin scheduling is not implimented, using FIFO\n";
   	   	processQueue = createFIFOQueue(cycle);
   	   }

	}
	/////////Removes the last 5 elements from cycle vector ////////////
	for(int i = 0; i < 5; i++)
		cycle.pop_back();


	
	/////////Checks that the memory block size is less than system memory to run applications ////////////
	...
	toPass->pcb.memBlock = memBlockSize;
		...
		if(cycle[0].id == "S(start" )
		{
			...
			{
				for(index = 0; index < processQueue.front().size(); index++)
				{
					//copying values to a struct to pass to the run process function
					toPass->m_time = processQueue.front()[index].cycleTime;
					toPass->operation = processQueue.front()[index].id;
					toPass->pcb.processState = processQueue.front()[index].pcb.processState;
					//start time of the operation
					timeTaken =  ( std::clock() - start ) / (double) CLOCKS_PER_SEC;
					printStart(timeTaken,counter,processQueue.front()[index].id, logToMon, logToFile, outputFile, toPass);
					//pthread funtions
					...
					if(addrChange == true)
					{
						...
						printEnd(timeTaken,counter,processQueue.front()[index].id, newData->address, logToMon, logToFile, outputFile);	//prints output	
					...
					else
					{
						...
						printEnd(timeTaken,counter,processQueue.front()[index].id, newData->address, logToMon, logToFile, outputFile);			
					}
				}
				//Incriment counter for applications
				counter++;
				//reset all resources because application finished
				toPass-> hardDriveNum[0] = 0;
				toPass-> hardDriveNum[1] = 0;
				toPass-> printerNum = 0;
				toPass->address = 0;
				processQueue.pop();
 ...
 *
 * @post Queue of vectors is returned based on FIFO CPU scheduling
 *
 */
 std::queue<std::vector<cycleInfo> > createFIFOQueue(std::vector<cycleInfo> cycle)
 {
 	////Variable Definitions //////////////
 	std::queue<std::vector<cycleInfo> > FIFOQueue;
 	std::vector<cycleInfo> vec;
 	int i = 0;
 	int j = 0;
 	
 	/////// Goes through the cycles vector to build a queue ///////////
 	for(int i =0; i < cycle.size(); i++)
 	{
 		if(cycle[i].id == "S(start")
 		{
 			//do nothing
 		}
 		
 		else if(cycle[i].id == "A(end")
 		{
 			vec.push_back(cycle[i]);
 			FIFOQueue.push(vec);
 			vec.clear();
...
*
* @details function used with sort to sort in ascending order by size
*
* @param in: 2 vectors 
*
* @pre 2 vectors of type cycle info are valid
*
* @post bool value returned based on comparison
*
*/

bool less_vectors(const std::vector<cycleInfo>& first, const std::vector<cycleInfo>& second)
 {
...
* @brief function to sort vector
*
* @details function used with sort to sort in descending order by size
*
* @param in: 2 vectors 
*
* @pre 2 vectors of type cycle info are valid
...
*
*/
 bool more_priority(const std::vector<cycleInfo>& first, const std::vector<cycleInfo>& second)
 {
 	int a=0;
 	int b = 0;
 	for(int i =0; i < first.size(); i++)
 	{
 		//incriments a if there is an I/O operation
 		if(first[i].id != "A(start" || first[i].id != "M(block" ||first[i].id != "M(allocate" || first[i].id != "P(run")
 			a++;

 	}
 	for(int i =0; i < second.size(); i++)
 	{
 		if(second[i].id != "A(start" || second[i].id != "M(block" ||second[i].id != "M(allocate" || second[i].id != "P(run")
 			b++;
 	}
 ...
 * @brief creates queue for Shorest Job First scheduling
 *
 * @details creates a queue that is in order of shortest job 
 *          
 * @param in: vector containing all of the applications
 *
 * @pre readData function has already been run
 *
 * @post Queue is returned based on SJF CPU scheduling
 *
 */
  std::queue<std::vector<cycleInfo> > createSJFqueue(std::vector<cycleInfo> cycle)
  {
  	////Variable Definitions //////////////
 	std::queue<std::vector<cycleInfo> > SJFQueue;
 	std::vector<std::vector<cycleInfo> > SJFVec;
 	std::vector<cycleInfo> vec;
 	int i = 0;
 	int j = 0;
 	
 	/////// Goes through the cycles vector to build a 2D vector so sort ///////////
 	for(int i =0; i < cycle.size(); i++)
 	{
 		if(cycle[i].id == "S(start" || cycle[i].id == "S(end")
 		{
 			//do nothing
 		}
 	...
 	//sort vector based on size of each vector
 	sort(SJFVec.begin(), SJFVec.end(), less_vectors);
 	
 	//copy sorted vector into queue
 	for(int i = 0; i < SJFVec.size(); i++)
 	{
 		SJFQueue.push(SJFVec[i]);
 	}
 	return SJFQueue;
 }
 ...
 /**
 * @brief creates queue for Priority scheduling
 *
 * @details creates a queue that is in order of most IO operations 
 *          
 * @param in: vector containing all of the applications
 *
 * @pre readData function has already been run
 *
 * @post Queue is returned based on PS CPU scheduling
 *
 */
 std::queue<std::vector<cycleInfo> > createPSqueue(std::vector<cycleInfo> cycle)
 {
 	//variable definitions/////////////////////////
 	std::vector<std::vector<cycleInfo> > PSVec;
 	std::vector<cycleInfo> vec;
 	std::queue<std::vector<cycleInfo> > PSQueue;
 	int nonIO = 0;//counter for 
 	...
 	sort(PSVec.begin(), PSVec.end(), more_priority);
 	for(int i =0; i < PSVec.size(); i++)
 	{
 		PSQueue.push(PSVec[i]);
 	}
 	return PSQueue;
